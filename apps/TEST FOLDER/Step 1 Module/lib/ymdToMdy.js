...
if ( {{ selectDateRange.value }} === "sevenDays" ){
...
} else if ( {{ selectDateRange.value }} === "thirtyDays" ){
    return {
    start : defaultStart30,
    end : defaultEnd
  }
} else if ( {{ selectDateRange.value }} === "ninetyDays" ){
...
} else if ( {{selectDateRange.value}} === "oneYear" ){
  const oneYearAgoMilli = Date.parse(new Date()) - ( 1000 * 60 * 60 * 24 * 365 );
...
if ( {{ selectDateRange.value }} === "customRange" ){
