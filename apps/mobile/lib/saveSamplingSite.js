...
//Process
let sampleData = trnSiteData.value;
...
if (isOffline)
{
  ...
  let queryString = upsert_soil_sampling.query;
  ...
  {    
    let newJob = await PendingJobClass.trigger({
      additionalScope:{
        ...
        jsonParameters: {
          ...
          entity_id: trnSiteData.value.id,
    ...
    //Update dependency
    if (samplingRoundOfflineJobId != undefined)
    {
      newJob.operationDependency.field_name = 'p_soil_sampling_round_id';
      newJob.operationDependency.jobId = samplingRoundOfflineJobId; //job id
    }
