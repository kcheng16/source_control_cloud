...
WHERE tbl_online.project_id = {{ varSelectedProject.value.project_id }} AND
  (
    ...
    --({varTempSoilRoundData.value.id }} IS NULL AND 
      (
        {{varTempSoilRoundData.value.sampling_type == 'type_yearly' }} AND COALESCE(tbl_online.year, '') = COALESCE({{ varTempSoilRoundData.value.year }}, '')
      )  or
      (
        {{varTempSoilRoundData.value.sampling_type == 'type_official' }} AND COALESCE(tbl_online.reporting_period, '') = COALESCE({{ varTempSoilRoundData.value.reporting_period }}, '')
      )
    --)
  )
...
WHERE tbl_offline.project_id = {{ varSelectedProject.value.project_id }} AND
  (
    --({varTempSoilRoundData.value.id}} IS NOT NULL AND COALESCE(id, '') = COALESCE({ varTempSoilRoundData.value.id }}, '')) OR
    --({varTempSoilRoundData.value.id }} IS NULL AND 
      (
        {{varTempSoilRoundData.value.sampling_type == 'type_yearly' }} AND COALESCE(tbl_offline.year, '') = COALESCE({{ varTempSoilRoundData.value.year }}, '')
      )  or
      (
        {{varTempSoilRoundData.value.sampling_type == 'type_official' }} AND COALESCE(tbl_offline.reporting_period, '') = COALESCE({{ varTempSoilRoundData.value.reporting_period }}, '')
      )
    --)
  ) and tbl_online.project_id IS NULL 
